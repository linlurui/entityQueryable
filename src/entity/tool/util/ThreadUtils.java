            ...
            for(FutureTask<List<T>> future : futureList) {
                try
                {
                    ...
                    try {
                        ...
                        log.error("ThreadUtils.sync: task execution exception", e);
                        throw e;
                    } catch (InterruptedException e) {
                        log.error("ThreadUtils.sync: interrupted while waiting for tasks", e);
                        // 取消未完成的任务
                    ...
                    log.error("batch task execution error: {}", e.getMessage(), e);
                }
            }
            log.debug("batch all tasks completed, data size: {}", futureList.size());
        }
    }
